...
import { procRenderSep, preview } from "./renderProc";
import { loadHdr } from "./textures/iniHdrTexture";

export class SkyboxWithHdr {
  ...
  loadHDR = async () => {
    await loadHdr("/textures/environment/out.hdr");
    preview();
    const result = await procRenderSep(2048);
    const images = await this.convertToImg(result.blobs);
  ...
  convertToImg = async data => {
    const images = [];
    if (data instanceof Array) {
      for (const item of data) {
        const img = await this.loadImgBlob(item);
        images.push(img);
      }
    }
